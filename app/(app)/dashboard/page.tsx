...
const QUICK_ACCESS = [
  ...
  { href: "/bible", icon: "✝️", label: "Bible", bg: "bg-amber-50 border-amber-100" },
  { href: "/journal?prayer=true", icon: "🙏", label: "Prayers", bg: "bg-purple-50 border-purple-100" },
...
export default async function DashboardPage() {
  ...
  const [profileRes, devotionalRes] = await Promise.all([
  ...
  // Build streak dot indicators (last 7 days)
  const streakDots = Array.from({ length: 7 }, (_, i) => i < streak % 7);

  return (
    <div className="flex flex-col gap-5 px-4 pt-10 pb-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="w-10 h-10 rounded-full bg-card border border-border/60 flex items-center justify-center shadow-sm">
      ...
      <div className="bloom-card">
        ...
        {devotional && !devotional.is_completed && (
          <Link
            href="/devotional"
            className="mt-3 flex items-center justify-center gap-2 w-full h-10 rounded-xl bg-primary/10 text-primary text-sm font-medium active:scale-[0.98] transition-transform"
          >
